...
function MissedCallSettingsPage() {
  ...
  if (error)
    return (
      <AppShell showCallBar={false} tenant={tenant} hidePublicNav>
        <div className="mx-auto max-w-3xl px-4 py-10 text-destructive">{error}</div>
      </AppShell>
    );
  if (!ctx)
    return (
      <AppShell showCallBar={false} tenant={tenant} hidePublicNav>
        <div className="mx-auto max-w-3xl px-4 py-10 text-muted-foreground">Loading…</div>
      </AppShell>
    );
  ...
  const save = async () => {
    setSaving(true);
    try {
      await updateMyMissedCallSettings({
        data: {
      ...
      <div className="mx-auto max-w-3xl px-4 py-6 space-y-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Configure the SMS your callers receive when you miss a call, and the alert you get
              when a lead comes back.
            </p>
          </div>
        ...
        <div className="rounded-md border border-border bg-card p-4 flex flex-wrap items-center gap-3 text-sm">
          ...
          <span className="rounded-full border border-border px-3 py-1 text-xs">
            Plan: <b>{ctx.business.selected_plan ?? "none"}</b>
          </span>
          <span className="rounded-full border border-border px-3 py-1 text-xs">
            {featureStateLabel[ctx.feature_state] ?? ctx.feature_state}
          </span>
          {ctx.business.trial_ends_at && (
            <span className="text-xs text-muted-foreground">
              Trial ends: {new Date(ctx.business.trial_ends_at).toLocaleDateString()}
            </span>
          )}
        </div>

        <div className="rounded-md border border-primary/40 bg-primary/5 p-3 text-sm">
          Text Link is currently <b>{s.enabled && s.mode === "live" ? "active" : "not active"}</b>.
          Routing is controlled from{" "}
          <Link to="/call-handling" className="font-bold text-primary underline">
            Call Handling
          </Link>
          , where only one inbound mode can run at a time.
        </div>
        ...
        {gated && (
          <div className="rounded-md border border-amber-500/40 bg-amber-500/10 p-3 text-sm flex items-start gap-2">
            ...
            <div>
              Your current plan/trial state does not include missed-call recovery. Enable it once
              you're on the Missed-Call Recovery or AI Receptionist plan.
            </div>
          </div>
        )}
        ...
        <section className="rounded-lg border border-border bg-card p-5 space-y-4">
          <div className="flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-muted-foreground">
            <Settings className="h-4 w-4" /> Recovery SMS
          </div>
          <label className="block">
            ...
            <div className="mt-1 text-xs text-muted-foreground">
              Variables: <code>{"{{business_name}}"}</code> <code>{"{{recovery_link}}"}</code>{" "}
        ...
        <section className="rounded-lg border border-border bg-card p-5 space-y-4">
          <div className="flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-muted-foreground">
            <PhoneCall className="h-4 w-4" /> Plumber alerts
          </div>
          <label className="flex items-center gap-2 text-sm">
            <input
              type="checkbox"
              checked={s.plumber_alert_enabled}
          ...
          <label className="block">
            ...
            <select
              value={s.alert_method}
              onChange={(e) => set({ alert_method: e.target.value as "demo" | "sms" | "email" })}
        ...
        <section className="rounded-lg border border-border bg-card p-5 space-y-4">
          <div className="flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-muted-foreground">
            <Send className="h-4 w-4" /> Send a test missed call
          </div>
          <p className="text-sm text-muted-foreground">
            Creates a real missed-call + SMS event scoped to your business only. In demo mode
          ...
          {testResult && (
            <div className="space-y-3">
              ...
              <DemoSmsPreview
                to={callerPhone}
                body={testResult.smsBody}
                status={testResult.simulated ? "simulated" : "sent"}
              />
              <div className="text-xs text-muted-foreground break-all">
                Recovery link:{" "}
                <a
                  className="underline"
                  href={testResult.recoveryLink}
                  target="_blank"
                  rel="noreferrer"
                >
                  {testResult.recoveryLink}
                </a>
              </div>
              <Link
                to="/b/$slug/request"
                params={{ slug: testResult.businessSlug }}
