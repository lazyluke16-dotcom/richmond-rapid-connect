...
export const Route = createFileRoute("/_authenticated/onboarding")({
  head: () => ({
    meta: [
      { title: "Set up your website — Rapid Connect" },
...
function OnboardingWizard() {
        ...
        if (status.hasBusiness) {
          ...
          if (bundle.services.length > 0) {
            ...
            const merged: ServiceSel[] = DEFAULT_SERVICES.map((d) => {
              ...
              return {
                key: d.key,
                name: hit?.display_name ?? d.name,
                enabled: hit ? hit.active : false,
              };
            });
    ...
    withBusy(async () => {
      await setMyServices({
        data: {
          services: services
            .filter((s) => s.enabled)
            .map((s) => ({ service_key: s.key, display_name: s.name })),
        },
      });
      await persistStep(3);
      setStep(3);
    ...
    withBusy(async () => {
      ...
      if (hasCoverageInput) {
        try {
          ...
          const msg = e instanceof Error ? e.message : String(e);
          const columnMissing =
            /(base_suburb|travel_radius_km|region_labels|postcode_ranges|excluded_areas|licence_|does not exist|not found|could not find|42703)/i.test(
              msg,
            );
          if (columnMissing) {
            throw new Error(
          ...
          throw new Error(
            `Could not save service coverage — ${msg}. Your existing settings were not overwritten.`,
          );
        }
      }
  ...
  const updateBiz = (patch: Partial<EditableBusiness>) =>
    setBiz((b) => (b ? { ...b, ...patch } : b));
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]">
        <div className="rounded-lg border border-border bg-card p-5">
          {step === 0 && (
            <div className="space-y-4">
              ...
              <Field
                ...
                placeholder="24/7 emergency plumbers based in Sydney"
                multiline
              />
          ...
          {step === 1 && biz && (
            <div className="space-y-4">
              <Field
                ...
                value={biz.logo_url ?? ""}
                onChange={(v) => updateBiz({ logo_url: v || null })}
                placeholder="https://…"
              />
              <p className="text-xs text-muted-foreground">
                Direct file upload is coming soon — paste a hosted URL for now (e.g. from your
                Google Drive share link or existing website).
              </p>
              <div className="grid grid-cols-3 gap-3">
                <ColorField
                  label="Primary"
                  value={biz.primary_colour ?? "#0EA5E9"}
                  onChange={(v) => updateBiz({ primary_colour: v })}
                />
                <ColorField
                  label="Secondary"
                  value={biz.secondary_colour ?? "#0B2545"}
                  onChange={(v) => updateBiz({ secondary_colour: v })}
                />
                <ColorField
                  label="Accent"
                  value={biz.accent_colour ?? "#67E8F9"}
                  onChange={(v) => updateBiz({ accent_colour: v })}
                />
              </div>
          ...
          {step === 2 && (
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground">
                Enable the services you offer. You can edit names.
              </p>
              {services.map((s, i) => (
                <div key={s.key} className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setServices(
                        services.map((x, j) => (j === i ? { ...x, enabled: e.target.checked } : x)),
                      )
                    }
                    className="h-5 w-5"
                  />
          ...
          {step === 3 && (
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground">
                Tell us where you're based and how far you'll travel. You can add broad region
                names, optional postcode ranges, and specific exclusions.
              </p>
              <div className="rounded-md border border-border bg-background/40 p-3 space-y-3">
                ...
                <ChipsField
                  ...
                  values={postcodeRanges}
                  setValues={setPostcodeRanges}
                  input={postcodeInput}
                  setInput={setPostcodeInput}
                  placeholder="3000-3199"
                />
                <ChipsField
                  label="Exclusions (optional)"
                  hint="Specific suburbs or areas you never service."
                  values={excludedAreas}
                  setValues={setExcludedAreas}
                  input={excludeInput}
              ...
              <details
                ...
                open={showAdvancedAreas}
                onToggle={(e) => setShowAdvancedAreas((e.currentTarget as HTMLDetailsElement).open)}
              >
                <summary className="cursor-pointer select-none px-3 py-2 text-sm font-bold">
                  Advanced: specific suburb list
                </summary>
                <div className="space-y-2 p-3 pt-1">
                  <p className="text-xs text-muted-foreground">
                    Optional — only use this if you need to enumerate every suburb explicitly.
                    Existing entries are preserved; leaving this alone won't overwrite them.
                  </p>
                  ...
                  <ul className="flex flex-wrap gap-2">
                    {areas.map((a) => (
                      ...
                      >
                        {a}
                        <button onClick={() => setAreas(areas.filter((x) => x !== a))}>
                          <X className="h-3 w-3" />
                        </button>
                      </li>
                    ))}
                    {areas.length === 0 && (
                      <li className="text-sm text-muted-foreground">No suburbs yet.</li>
                    )}
                  </ul>
                </div>
              </details>
                ...
                <div key={h.day_of_week} className="flex items-center gap-3 py-1">
                  ...
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      ...
                      onChange={(e) =>
                        setHours(
                          hours.map((x, j) => (j === i ? { ...x, closed: !e.target.checked } : x)),
                        )
                      }
                    />
                    Open
                    ...
                    <>
                      <input
                        ...
                        onChange={(e) =>
                          setHours(
                            hours.map((x, j) =>
                              j === i ? { ...x, open_time: e.target.value } : x,
                            ),
                          )
                        }
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setHours(
                            hours.map((x, j) =>
                              j === i ? { ...x, close_time: e.target.value } : x,
                            ),
                          )
                        }
                        className="rounded-md border border-border bg-input px-2 py-1 text-sm"
          ...
          {step === 5 && (
            <div className="space-y-4">
              <PlanCard
                selected={plan === "missed_call_recovery"}
                onSelect={() => setPlan("missed_call_recovery")}
                icon={<Sparkles className="h-5 w-5" />}
                title="Missed Call Recovery"
                bullets={[
                  "Branded job-capture website",
                ...
                <div className="rounded-md border border-yellow-500/40 bg-yellow-500/5 p-3 text-xs text-yellow-200 flex gap-2">
                  ...
                  <div>
                    ...
                    <p className="mt-1 opacity-90">
                      Your choice is saved. The AI receptionist is <b>included</b> in your setup but
                      is <b>not activated</b>, <b>not billable</b>, and setup is <b>incomplete</b>.
                      No telephony is provisioned and no payment method is connected. A future Go
                      Live step will handle activation.
                    </p>
                  </div>
                </div>
              )}
          ...
          {step === 6 && biz && (
            <div className="space-y-4">
              <Field
                ...
                onChange={(v) => updateBiz({ hero_heading: v || null })}
                placeholder="Sydney's fastest plumbers"
              />
              <Field
                label="Hero subheading"
                value={biz.hero_subheading ?? ""}
                onChange={(v) => updateBiz({ hero_subheading: v || null })}
                multiline
              />
              <Field
                label="Emergency message"
                value={biz.emergency_message ?? ""}
                onChange={(v) => updateBiz({ emergency_message: v || null })}
          ...
          {step === 7 && biz && (
            <div className="text-center py-8">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Your public job-capture site is live at:
              </p>
        ...
        <aside className="rounded-lg border border-border bg-card p-4 h-fit sticky top-6">
          ...
          {biz && (
            <div className="mt-3 text-[11px] text-muted-foreground">
              Slug: <span className="font-mono">{biz.slug}</span>
            </div>
          )}
          <div className="mt-3 text-[11px]">
            <Link to="/dashboard" className="text-muted-foreground underline">
              Skip to dashboard (finishes later)
            </Link>
          </div>
        </aside>
      </div>
  ...
  return (
    <div className="flex items-center justify-between pt-2">
      ...
      <button
        ...
        className="inline-flex items-center gap-2 rounded-md bg-primary px-5 py-3 font-black text-primary-foreground disabled:opacity-40"
      >
    ...
    <div>
      ...
      <ul className="mt-2 flex flex-wrap gap-2">
        {values.map((a) => (
          ...
          >
            {a}
            <button type="button" onClick={() => setValues(values.filter((x) => x !== a))}>
              <X className="h-3 w-3" />
            </button>
          </li>
        ))}
...
function PreviewCard({
  ...
  return (
    <div
      ...
      style={{ background: `linear-gradient(180deg, ${secondary} 0%, #000 100%)` }}
    >
      <div className="flex items-center gap-2">
        {logo ? (
      ...
      <div className="mt-3 text-lg font-black" style={{ color: primary }}>
        {hero}
      </div>
      <p className="mt-1 text-xs text-white/70">{subhero}</p>
      <span
        className="mt-2 inline-block rounded px-2 py-1 text-xs font-black"
        style={{ background: primary }}
      >
        Start job request
      </span>
      {services.length > 0 && (
        <div className="mt-3">
          ...
          <div className="mt-1 flex flex-wrap gap-1">
            {services.map((s) => (
              <span key={s} className="rounded bg-white/10 px-2 py-0.5 text-[11px]">
                {s}
              </span>
            ))}
          </div>
      ...
      {areas.length > 0 && (
        <div className="mt-3">
          ...
          <div className="mt-1 flex flex-wrap gap-1">
            {areas.map((a) => (
              <span key={a} className="rounded bg-white/10 px-2 py-0.5 text-[11px]">
                {a}
              </span>
            ))}
          </div>
