...
export async function handleAiPhoneLead(request: Request): Promise<Response> {
  ...
  if (body.integration_token) {
    ...
    const { data: m } = await supabaseAdmin
      ...
      .select("business_id")
      .eq("integration_token_hash", tokenHash)
      .eq("active", true)
      .maybeSingle();
  ...
  const lead: Lead = {
    ...
    chat: body.transcript ? [{ role: "ai", text: body.transcript, ts: Date.now() }] : [],
    aiSummary: "",
    leadScore: 0,
    recommendedAction: "",
    status: "new",
  ...
  if (error) {
    if (/duplicate key|leads_source_external_call_uk/i.test(error.message)) {
      return new Response(JSON.stringify({ ok: true, deduped: true, lead_id: lead.id }), {
        status: 200,
        headers: { "Content-Type": "application/json" },
      });
    }
