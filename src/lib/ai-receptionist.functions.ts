...
export interface AiContextPreview {
  business: { id: string; name: string; slug: string; public_phone: string | null; selected_plan: string | null; trial_ends_at: string | null };
...
export function buildReceptionistInstructions(input: {
  ...
  areas: { name: string }[];
  hours: { day: number; open: string | null; close: string | null; closed: boolean }[];
  settings: Pick<AiReceptionistSettings,
    'assistant_name' | 'first_message' | 'tone' | 'language' |
    'callback_message' | 'pricing_response' | 'human_request_response' | 'emergency_response'>;
}): string {
  const days = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
  const hoursLine = input.hours.length
    ? input.hours.map(h => `${days[h.day] ?? h.day}: ${h.closed ? 'closed' : `${h.open ?? '?'}–${h.close ?? '?'}`}`).join(', ')
    : 'not configured';
...
async function loadOwnBusiness(supabase: { from: (t: string) => { select: (s: string) => { limit: (n: number) => { maybeSingle: () => Promise<{ data: unknown; error: { message: string } | null }> } } } }) {
  const { data, error } = await supabase.from('businesses').select('id,name,slug,public_phone,selected_plan,trial_ends_at').limit(1).maybeSingle();
  if (error) throw new Error(error.message);
  if (!data) throw new Error('No business membership');
  return data as { id: string; name: string; slug: string; public_phone: string | null; selected_plan: string | null; trial_ends_at: string | null };
}
...
export const getMyAiContext = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ context }): Promise<AiContextPreview> => {
    ...
    let { data: s } = await context.supabase
      .from('business_ai_receptionist_settings').select('*').eq('business_id', biz.id).maybeSingle();
    if (!s) {
      const { data: ins } = await context.supabase
        .from('business_ai_receptionist_settings').insert({ business_id: biz.id } as never).select('*').single();
      s = ins;
    }
    const settings = s as unknown as AiReceptionistSettings;

    const [{ data: services }, { data: areas }, { data: hours }, { data: fs }, { data: acc }] = await Promise.all([
      context.supabase.from('business_services').select('service_key,display_name').eq('business_id', biz.id),
      context.supabase.from('business_service_areas').select('suburb').eq('business_id', biz.id),
      context.supabase.from('business_hours').select('day_of_week,open_time,close_time,closed').eq('business_id', biz.id),
    ...
    const svcs = ((services ?? []) as unknown as { service_key: string; display_name: string }[]).map(r => ({ key: r.service_key, label: r.display_name }));
    const ars = ((areas ?? []) as unknown as { suburb: string }[]).map(r => ({ name: r.suburb }));
    const hrs = ((hours ?? []) as unknown as { day_of_week: number; open_time: string | null; close_time: string | null; closed: boolean }[])
...
export type AiSettingsUpdate = Partial<Pick<AiReceptionistSettings,
  'enabled' | 'mode' | 'assistant_name' | 'first_message' | 'tone' | 'language' |
  'callback_message' | 'pricing_response' | 'human_request_response' | 'emergency_response' |
  'recording_enabled' | 'transcript_enabled' | 'ai_summary_enabled' | 'max_call_duration_seconds'>>;
...
export const updateMyAiSettings = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.assistant_name !== undefined) { const v = clean(data.assistant_name, 60); if (v.length < 2) throw new Error('Assistant name too short'); patch.assistant_name = v; }
    if (data.first_message !== undefined) { const v = clean(data.first_message, 240); if (v.length < 5) throw new Error('First message too short'); patch.first_message = v; }
    ...
    if (data.human_request_response !== undefined) patch.human_request_response = clean(data.human_request_response);
    if (data.emergency_response !== undefined) patch.emergency_response = clean(data.emergency_response);
    ...
    if (data.max_call_duration_seconds !== undefined) {
      ...
      if (!Number.isFinite(n) || n < 30 || n > 900) throw new Error('max_call_duration_seconds must be 30–900');
    ...
    if (patch.enabled === true) {
      const { data: acc } = await context.supabase.rpc('has_ai_receptionist_access', { _business_id: biz.id });
    ...
    if (patch.mode === 'live') {
      ...
      const { data: mapping } = await supabaseAdmin.from('ai_provider_mappings')
        .select('id').eq('business_id', biz.id).eq('active', true).limit(1).maybeSingle();
      if (!mapping) patch.mode = 'demo';
    }

    const { error } = await context.supabase
      .from('business_ai_receptionist_settings').update(patch as never).eq('business_id', biz.id);
    if (error) throw new Error(error.message);
    return { success: true };
  });
...
function normaliseJobType(input: string | undefined, allowed: string[]): JobType {
  ...
  if (/burst|leak|flood/.test(s)) return (allowed.includes('burst-pipe') ? 'burst-pipe' : 'other') as JobType;
  if (/block|drain|sewer/.test(s)) return (allowed.includes('blocked-drain') ? 'blocked-drain' : 'other') as JobType;
  if (/hot ?water|no hot/.test(s)) return (allowed.includes('hot-water') ? 'hot-water' : 'other') as JobType;
...
export const simulateAiCall = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: acc } = await context.supabase.rpc('has_ai_receptionist_access', { _business_id: biz.id });
    if (!acc) throw new Error('Plan does not include AI receptionist — choose the AI receptionist plan or renew your trial.');

    const { data: svcRows } = await context.supabase
      .from('business_services').select('service_key').eq('business_id', biz.id);
