...
export type MissedCallUpdate = Partial<Pick<
  MissedCallSettings,
  'enabled' | 'mode' | 'recovery_sms_enabled' | 'sms_template' |
  'plumber_alert_enabled' | 'alert_method' | 'alert_phone' | 'alert_email' | 'callback_message'
>>;

function sanitizeTemplate(t: string): string {
  // Strip HTML tags & control chars. Keep it plain SMS text.
  return t.replace(/<[^>]*>/g, '').replace(/[\u0000-\u0008\u000B-\u001F]/g, '').slice(0, 480);
}
...
export const updateMyMissedCallSettings = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.recovery_sms_enabled !== undefined) patch.recovery_sms_enabled = !!data.recovery_sms_enabled;
    if (data.plumber_alert_enabled !== undefined) patch.plumber_alert_enabled = !!data.plumber_alert_enabled;
    if (data.alert_method !== undefined) patch.alert_method = ['demo','sms','email'].includes(data.alert_method) ? data.alert_method : 'demo';
    if (data.alert_phone !== undefined) patch.alert_phone = data.alert_phone ? String(data.alert_phone).slice(0, 40) : null;
    if (data.alert_email !== undefined) patch.alert_email = data.alert_email ? String(data.alert_email).slice(0, 200) : null;
    if (data.callback_message !== undefined) patch.callback_message = data.callback_message ? sanitizeTemplate(data.callback_message) : null;
    ...
    if (patch.enabled === true) {
      ...
      const { data: acc } = await context.supabase.rpc('has_missed_call_access', { _business_id: biz.id });
    ...
    const { error } = await context.supabase
      ...
      .eq('business_id', (await context.supabase.from('businesses').select('id').limit(1).single()).data!.id);
...
export const sendTestMissedCall = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }): Promise<TestMissedCallResult> => {
    ...
    const { data: acc } = await context.supabase.rpc('has_missed_call_access', { _business_id: biz.id });
    if (!acc) throw new Error('Plan does not include missed-call recovery — choose a plan or renew your trial.');
    ...
    const effectiveMode: 'demo' | 'live' = settings.mode === 'live' && smsMode === 'twilio' ? 'live' : 'demo';

    const mcid = crypto.randomUUID();
    const baseUrl = (data.baseUrl && /^https?:\/\//.test(data.baseUrl))
    ...
    const { error: sErr } = await context.supabase.from('sms_events').insert({
      ...
      body: smsBody,
      mode: effectiveMode === 'live' ? 'twilio' : 'demo',
      status: effectiveMode === 'live' ? 'sent' : 'simulated',
