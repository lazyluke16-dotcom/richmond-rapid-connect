 ...
 */
export function isMissingOnboardingStepError(err: { message?: string; code?: string } | null | undefined): boolean {
  if (!err) return false;
  if (err.code === '42703') return true;
  const m = (err.message ?? '').toLowerCase();
  return m.includes('onboarding_step') && (m.includes('does not exist') || m.includes('not found') || m.includes('could not find'));
}
...
export const getOnboardingStatus = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ context }): Promise<OnboardingStatus> => {
    ...
    const [{ count: servicesCount }, { count: areasCount }, { count: hoursCount }] = await Promise.all([
      context.supabase.from('business_services').select('id', { count: 'exact', head: true }).eq('business_id', bid),
      context.supabase.from('business_service_areas').select('id', { count: 'exact', head: true }).eq('business_id', bid),
      context.supabase.from('business_hours').select('day_of_week', { count: 'exact', head: true }).eq('business_id', bid),
    ]);
...
export interface OnboardingBundle {
  ...
  hours: { day_of_week: number; closed: boolean; open_time: string | null; close_time: string | null }[];
}
export const getMyOnboardingBundle = createServerFn({ method: 'GET' })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }): Promise<OnboardingBundle> => {
    const { data: biz } = await context.supabase.from('businesses').select('id').limit(1).maybeSingle();
    ...
    return {
      services: ((svcRes.data ?? []) as { service_key: string; display_name: string; active: boolean }[]).map((s) => ({
        service_key: s.service_key,
        display_name: s.display_name,
        active: s.active !== false,
      })),
      areas: ((areaRes.data ?? []) as { suburb: string; state: string | null; postcode: string | null }[]).map((a) => ({
        suburb: a.suburb,
        state: a.state ?? null,
        postcode: a.postcode ?? null,
      })),
      hours: ((hourRes.data ?? []) as { day_of_week: number; closed: boolean; open_time: string | null; close_time: string | null }[]).map((h) => ({
...
export const createMyBusiness = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }): Promise<{ id: string; slug: string }> => {
    const { data: rows, error } = await context.supabase.rpc('create_business_for_current_user' as never, {
      p_name: data.name,
...
export const setMyServices = createServerFn({ method: 'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: { services: { service_key: string; display_name: string; active?: boolean }[] }) => data)
  .handler(async ({ data, context }) => {
    const parsed = ServicesPayloadSchema.parse(data);
    const { data: biz, error: bErr } = await context.supabase.from('businesses').select('id').limit(1).maybeSingle();
    if (bErr) throw new Error(bErr.message);
    if (!biz?.id) throw new Error('No business');
    const bid = biz.id as string;
...
export const setMyAreas = createServerFn({ method: 'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: { areas: { suburb: string; state?: string | null; postcode?: string | null }[] }) => data)
  .handler(async ({ data, context }) => {
...
export const setMyHours = createServerFn({ method: 'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: { hours: { day_of_week: number; closed: boolean; open_time?: string | null; close_time?: string | null }[] }) => data)
  .handler(async ({ data, context }) => {
...
export const updateMySlug = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }): Promise<{ slug: string }> => {
    const { data: rows, error } = await context.supabase.rpc('update_my_business_slug' as never, { p_new_slug: data.slug } as never);
    if (error) throw new Error(error.message);
