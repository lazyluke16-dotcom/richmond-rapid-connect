...
describe('Payment recovery restores active state', () => {
  it('clearGraceAndActivate sets expected fields', async () => {
    ...
    await clearGraceAndActivate('biz-123', mockSupabase as unknown as Parameters<typeof clearGraceAndActivate>[1]);
...
describe('Duplicate Stripe webhook handling', () => {
  it('unique constraint on stripe_event_id prevents double-processing', () => {
    const insertError = { message: 'duplicate key value violates unique constraint "stripe_webhook_events_stripe_event_id_key"' };
...
describe('Stripe client security guards', () => {
  it('live-mode key throws before creating client', () => {
    const key = 'sk_live_XXXXXXXX';
    const guard = () => {
      if (key.startsWith('sk_live_')) throw new Error('Live-mode Stripe keys are not permitted');
    };
    expect(guard).toThrow('Live-mode Stripe keys are not permitted');
  });

  it('missing key throws descriptive error', () => {
    const guard = (key: string | undefined) => {
      if (!key) throw new Error('STRIPE_SECRET_KEY is not configured');
    };
    expect(() => guard(undefined)).toThrow('STRIPE_SECRET_KEY is not configured');
  });

  it('test key passes the guard', () => {
    const guard = (key: string) => {
      if (key.startsWith('sk_live_')) throw new Error('Live key');
    };
    expect(() => guard('sk_test_xxx')).not.toThrow();
  });
});
...
describe('Grace period timestamps', () => {
  it('setGracePeriod sets grace_expires_at to now + 48h on first failure', async () => {
      ...
      from: vi.fn().mockImplementation(() => ({
        select: vi.fn().mockReturnValue({
          eq: vi.fn().mockReturnValue({
            maybeSingle: vi.fn().mockResolvedValue({ data: { grace_started_at: null }, error: null }),
          }),
        }),
    ...
    await setGracePeriod('biz-456', mockSupabase as unknown as Parameters<typeof setGracePeriod>[1]);
  ...
  it('setGracePeriod preserves original grace_started_at on subsequent payment failures', async () => {
    ...
    const mockSupabase = {
      from: vi.fn().mockImplementation(() => ({
        select: vi.fn().mockReturnValue({
          eq: vi.fn().mockReturnValue({
            maybeSingle: vi.fn().mockResolvedValue({ data: { grace_started_at: originalStart }, error: null }),
          }),
        }),
        update: vi.fn().mockImplementation((data: Record<string, unknown>) => {
          captured.push(data);
    ...
    const { setGracePeriod } = await import('../billing.server');
    await setGracePeriod('biz-789', mockSupabase as unknown as Parameters<typeof setGracePeriod>[1]);
...
describe('Missing Stripe price configuration fails closed', () => {
  ...
  it('error message identifies ALL missing variables at once', () => {
    let errorMessage = '';
    try { getStripePrices(); } catch (e) { errorMessage = (e as Error).message; }
...
describe('Grace usage A$10 cap suspends on breach', () => {
  it('shouldSuspend is true when grace usage exceeds A$10', async () => {
    const usageRows = [
      { estimated_customer_charge: 5.50 },
      { estimated_customer_charge: 4.61 },
    ];
    const chain = { eq: vi.fn(), gte: vi.fn().mockResolvedValue({ data: usageRows, error: null }) };
    chain.eq.mockReturnValue(chain);
    const mockSupabase = { from: vi.fn().mockReturnValue({ select: vi.fn().mockReturnValue(chain) }) };
  ...
  it('A$10.00 grace usage — suspend immediately (at cap boundary, inclusive)', async () => {
    const usageRows = [{ estimated_customer_charge: 10.00 }];
    const chain = { eq: vi.fn(), gte: vi.fn().mockResolvedValue({ data: usageRows, error: null }) };
    chain.eq.mockReturnValue(chain);
    const mockSupabase = { from: vi.fn().mockReturnValue({ select: vi.fn().mockReturnValue(chain) }) };
    ...
    expect(result.shouldSuspend).toBe(true);
    expect(result.withinCap).toBe(false);
    expect(result.currentUsageAud).toBeCloseTo(10.00, 2);
  });
