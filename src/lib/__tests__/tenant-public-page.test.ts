...
let mockBundle: TenantBundle | null = null;
vi.mock("@tanstack/react-router", () => {
  ...
  const Outlet = () => null;
  const createFileRoute = (_path: string) => (_config: unknown) => ({
...
describe("Tenant public page rendering (src/routes/b.$slug.tsx → TenantHome)", () => {
  ...
  it("omits all licence details when licence_public is undefined (pre-migration schema)", () => {
    const b = makeBundle();
    delete (b.business as Partial<TenantBundle["business"]>).licence_public;
    const html = render(b);
    expect(html).not.toContain("LIC-12345");
    expect(html).not.toContain("Alice Holder");
    expect(html).not.toContain("2030-01-01");
  ...
  it("omits all licence details when licence_public is a non-boolean truthy value", () => {
    const truthies: unknown[] = [1, "true", "yes", {}, []];
    for (const v of truthies) {
      const html = render(
        makeBundle({
          licence_public: v as TenantBundle["business"]["licence_public"],
        }),
      );
      expect(html, `truthy value ${JSON.stringify(v)} must not expose licence`).not.toContain(
        "LIC-12345",
      );
      expect(html).not.toContain("Alice Holder");
      expect(html).not.toContain("2030-01-01");
...
describe("public business loader and pending-view disclosure contract", () => {
  ...
  it("normalises the pending licence projection and preserves the exact-true gate", () => {
    const business = parsePublicBusiness(
      rawPublicBusiness({
  ...
  it("keeps the committed view projection allowlisted and gates private licence values", () => {
    const sql = readFileSync(
      resolve(
        process.cwd(),
        "supabase/migrations/20260722201335_2f2ef531-beda-47b2-8745-979b1da71158.sql",
      ),
      "utf8",
    );
    ...
    expect(view, "committed migration must define the security-invoker public view").not.toBeNull();
    ...
    expect(sql).toMatch(/GRANT SELECT ON public\.businesses_public TO anon, authenticated;/i);
