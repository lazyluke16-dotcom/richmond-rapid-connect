...
export function usageChargeMinor(row: UsageChargeRow): number {
  if (
    row.estimated_customer_charge_minor !== null &&
    row.estimated_customer_charge_minor !== undefined
  ) {
    const minor = Number(row.estimated_customer_charge_minor);
    if (Number.isSafeInteger(minor) && minor >= 0) return minor;
  }

  const major = Number(row.estimated_customer_charge);
  return Number.isFinite(major) && major >= 0 ? Math.round(major * 100) : 0;
}

export function sumUsageChargesMinor(rows: UsageChargeRow[]): number {
  return rows.reduce((sum, row) => sum + usageChargeMinor(row), 0);
}
...
export async function setGracePeriod(
  ...
  const existingGraceStart = (existing as { grace_started_at?: string | null } | null)
    ?.grace_started_at;
  ...
  await supabaseAdmin.from("business_billing").update(updatePayload).eq("business_id", businessId);
}
...
export async function checkGraceUsageCap(
  ...
  const total = sumUsageChargesMinor((data ?? []) as UsageChargeRow[]) / 100;
...
export async function getCurrentPeriodUsage(
  ...
  let query = supabaseAdmin
    .from("billing_usage_events")
    .select(
      "estimated_customer_charge, estimated_customer_charge_minor, billable_seconds, stripe_meter_event_status",
    )
    .eq("business_id", businessId)
    .eq("billable", true);
  ...
  const totalCents = sumUsageChargesMinor(rows);
...
export async function getUnsentAlerts(
  ...
  const sentSet = new Set(
    ((sent ?? []) as { threshold_aud: number }[]).map((r) => r.threshold_aud),
  );
