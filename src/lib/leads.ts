export type JobType =
  "burst-pipe" | "blocked-drain" | "hot-water" | "leaking-tap" | "toilet" | "gas" | "other";
...
export const JOB_TYPES: { value: JobType; label: string; icon: string; blurb: string }[] = [
  {
    ...
    icon: "💥",
    blurb: "Water where it shouldn't be",
  },
  {
    value: "blocked-drain",
    label: "Blocked drain",
    icon: "🌀",
    blurb: "Sink, shower, sewer or stormwater",
  },
  {
    value: "hot-water",
    label: "No hot water",
    icon: "🔥",
    blurb: "Cold showers, pilot out, tank leaking",
  },
  {
    value: "leaking-tap",
    label: "Leaking tap or fitting",
    icon: "💧",
    blurb: "Drip, spray or loose tap",
  },
  {
    value: "toilet",
    label: "Toilet problem",
    icon: "🚽",
    blurb: "Blocked, running or won't flush",
  },
...
export type Draft = Partial<
  Omit<
    Lead,
    "id" | "createdAt" | "chat" | "aiSummary" | "leadScore" | "recommendedAction" | "status"
  >
> & {
  chat?: ChatMessage[];
};
...
export function scoreLead(
  l: Pick<Lead, "urgency" | "jobType" | "photos" | "phone" | "propertyType">,
): number {
  let score = 40;
  ...
  const demo: Lead[] = [
    {
      ...
      chat: [
        {
          role: "ai",
          text: "Have you turned the water off at the mains?",
          ts: now - 1000 * 60 * 9,
        },
        {
          role: "customer",
          text: "Yes, mains is off. Water was pouring out from under the kitchen sink.",
          ts: now - 1000 * 60 * 8,
        },
        {
          role: "ai",
          text: "Great job. Any damage to the cabinet or floor?",
          ts: now - 1000 * 60 * 8,
        },
        {
          role: "customer",
          text: "Cabinet base is soaked and it's spreading onto the timber floor.",
          ts: now - 1000 * 60 * 7,
        },
      ],
      aiSummary:
    ...
    },
    {
      ...
      chat: [
        {
          role: "ai",
          text: "Is your hot water system gas, electric or heat pump?",
          ts: now - 1000 * 60 * 56,
        },
        {
          role: "customer",
          text: "Gas, external. Probably 12 years old.",
          ts: now - 1000 * 60 * 55,
        },
        { role: "ai", text: "Any water pooling at the base?", ts: now - 1000 * 60 * 55 },
        {
          role: "customer",
          text: "Small puddle underneath, pilot won't stay lit.",
          ts: now - 1000 * 60 * 54,
        },
      ],
      aiSummary:
    ...
    },
    {
      ...
      chat: [
        ...
        {
          role: "customer",
          text: "Shower is draining super slowly, been getting worse over a week.",
          ts: now - 1000 * 60 * 188,
        },
      ],
      aiSummary:
